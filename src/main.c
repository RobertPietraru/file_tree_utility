...
int get_directory_name_index(char *folder_path) {
  int last_slash = -1;
  int i = 0;
  while (folder_path[i] != '\0') {
    if (folder_path[i] == '/')
      last_slash = i;
    i++;
  }
...
void show_structure(char *cwd, char *leading, int depth, int max_depth) {
  ...
  if (dir) {
    ...
    if (stat(full_path, &file_stat) == 0 && S_ISDIR(file_stat.st_mode)) {
      printf("─ %s/\n", entry->d_name);
      show_structure(full_path, next_leading_without_bar, depth + 1, max_depth);
    } else {
      printf("─ %s\n", entry->d_name);
    }
...
int main(int argc, char *argv[]) {
  ...
  char *dir = (char *)malloc(strlen(cwd) + 1);
  strcpy(dir, cwd);
  int index = get_directory_name_index(dir);
  dir += index;
  printf("%s\n", dir);
  dir -= index;
  free(dir);
